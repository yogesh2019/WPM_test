...
function initTimer(){
    ...
    // upon completion of test remove hidden class from modal showing result
    if(timeLeft < 1){
    let correctTyped = (parseInt(cpmTag.innerText) - (parseInt(errorTag.innerText) + errors));
    
   let accuracyVal = ((correctTyped / parseInt(cpmTag.innerText)) * 100)
       accuracy = Math.round(accuracyVal);

        scoreText.innerHTML = `Typing Test Complete!
        You typed the 1 Minute Typing Test.
        Your speed was <span>${wpmTag.innerHTML} WPM</span> with <span>${accuracy}%</span> accuracy!`

      modal.classList.remove("hidden");
    }
}
//upon clicking on the try again btn resetGame
